...
function Modal({ children, type, titulo, subtitulo }) {
  ...
  async function handleClickFetch(e) {
    ...
    setLoading(true)
    try {
      const responce = await fetch('http://192.168.0.107:21037/' + typeModal, {
        method: 'POST',
  ...
  if (modalTipo === "login") {
    return <Login onclickForm={(e) => handleClickFetch(e)} erroFetch={erroFetch} setModalTipo={setModalTipo} loading={loading} setErroFetch={setErroFetch} onChange={(e) => changeInput(e)} values={inputData}/>
  } else if (modalTipo === "cadastrar") {
    return <Cadastrar onclickForm={(e) => handleClickFetch(e)} erroFetch={erroFetch} setModalTipo={setModalTipo} setErroFetch={setErroFetch} loading={loading} onChange={(e) => changeInput(e)} values={inputData}/>
  }
